...
class InteractionAudioTests(unittest.TestCase):
    def test_variants_remain_paired(self):
        ...
        messages = []
        scope = {'store': SimpleNamespace(append_message=lambda role, text: messages.append((role, text))), 'random': random}
        ...
        with patch.object(random, 'choice', return_value=variant):
            self.assertEqual(scope['SpecialInteraction'](2), {'response': 'Example text', 'audio_url': '/audio/example.wav'})
        messages.clear()
